...
class Controller {
  customFaceGeometries: any[];
  estimator: Estimator;
  lastEstimateResult: any;
  filterMinCF: number;
  filterBeta: number;
  onUpdate: Function;
  flipFace: boolean;
  landmarkFilters: OneEuroFilter[];
  faceMatrixFilter: OneEuroFilter;
  faceScaleFilter: OneEuroFilter;
  ...
  async dummyRun(input: FaceLandmarkerResult) {
  ...
  processVideo(input: HTMLVideoElement) {
    ...
    const doProcess = async () => {
      ...
      if (this.flipFace) {
        flippedInputContext.clearRect(
          0,
          0,
          flippedCanvasElement.width,
          flippedCanvasElement.height,
        );
        flippedInputContext.save();
        flippedInputContext.translate(flippedCanvasElement.width, 0);
        flippedInputContext.scale(-1, 1);
        flippedInputContext.drawImage(
          input,
          0,
          0,
          flippedCanvasElement.width,
      ...
      if (results.faceLandmarks.length === 0) {
        ...
        if (this.lastEstimateResult === null) {
          ...
          const newFaceMatrix = this.faceMatrixFilter.filter(
            Date.now(),
            estimateResult.faceMatrix,
          );

          const newFaceScale = this.faceScaleFilter.filter(Date.now(), [
        ...
        if (this.onUpdate) {
          this.onUpdate({
            hasFace: true,
            estimateResult: this.lastEstimateResult,
          });
        }

        for (let i = 0; i < this.customFaceGeometries.length; i++) {
          this.customFaceGeometries[i].updatePositions(
            estimateResult.metricLandmarks,
          );
        }
      }
