...
//
import {
  positions as canonicalMetricLandmarks,
  landmarkBasis,
} from './face-data.js';
...
for (let i = 0; i < canonicalMetricLandmarks.length; i++) {
  if (
    canonicalMetricLandmarks[i][0] <
    canonicalMetricLandmarks[leftMostLandmarkIndex][0]
  ) {
    leftMostLandmarkIndex = i;
  }
  if (
    canonicalMetricLandmarks[i][0] >
    canonicalMetricLandmarks[rightMostLandmarkIndex][0]
  ) {
...
class Estimator {
  ...
  estimate(landmarks: number[][]) {
    // in new mediapipe version, there is 10 extra landmarks for iris (which we don't have in canonical model)
    ...
    for (let l = 0; l < metricLandmarks.length; l++) {
      ...
      for (let k = 0; k < 3; k++) {
        ...
        for (let i = 0; i < 3; i++) {
          newMetricLandmarks[l][k] +=
            invPoseTransformMat[k][i] * metricLandmarks[l][i];
        }
      }
    }
    ...
    majorLandmarkIndexes.forEach((idx) => {
      ...
      imagePoints.push(
        landmarks[idx][0] * this.frameWidth,
        landmarks[idx][1] * this.frameHeight,
      );
    });
    //console.log("estimator modelPoints", modelPoints);
    //console.log("estimator imagePoints", imagePoints);

    const modelPointsMat = opencv.matFromArray(
      modelPoints.length / 3,
      3,
      opencv.CV_64F,
      modelPoints,
    );
    const imagePointsMat = opencv.matFromArray(
    ...
    const faceScale =
      newMetricLandmarks[rightMostLandmarkIndex][0] -
      newMetricLandmarks[leftMostLandmarkIndex][0];
  ...
  _solveWeightedOrthogonal(sources, targets, sqrtWeights) {
    ...
    for (let k = 0; k < 3; k++) {
      for (let l = 0; l < 3; l++) {
        for (let i = 0; i < weightedTargets.length; i++) {
          designMatrix[k][l] +=
            weightedTargets[i][k] * centeredWeightedSources[i][l];
        }
      }
    }
    ...
    for (let l = 0; l < weightedSources.length; l++) {
      ...
      for (let k = 0; k < 3; k++) {
        ...
        for (let i = 0; i < 3; i++) {
          pointwiseDiffs[l][k] -=
            rotationAndScale[k][i] * weightedSources[l][i];
        }
      }
    }
    ...
    const transformMat = [
      [
        rotationAndScale[0][0],
        rotationAndScale[0][1],
        rotationAndScale[0][2],
        translation[0],
      ],
      [
        rotationAndScale[1][0],
        rotationAndScale[1][1],
        rotationAndScale[1][2],
        translation[1],
      ],
      [
        rotationAndScale[2][0],
        rotationAndScale[2][1],
        rotationAndScale[2][2],
        translation[2],
      ],
      [0, 0, 0, 1],
    ];
  ...
  _computeOptimalScale(
    ...
    for (let l = 0; l < centeredWeightedSources.length; l++) {
      ...
      for (let k = 0; k < 3; k++) {
        ...
        for (let i = 0; i < 3; i++) {
          rotatedCenteredWeightedSources[l][k] +=
            rotation[k][i] * centeredWeightedSources[l][i];
        }
      }
    }
    let numerator = 0;
    for (let l = 0; l < rotatedCenteredWeightedSources.length; l++) {
      for (let k = 0; k < 3; k++) {
        numerator +=
          rotatedCenteredWeightedSources[l][k] * weightedTargets[l][k];
      }
    }
    let denominator = 0;
    for (let l = 0; l < centeredWeightedSources.length; l++) {
